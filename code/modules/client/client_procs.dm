...
/client/verb/link_discord_account()
	...
	if(discord_id && length(discord_id) < 32)
		to_chat(usr, "<span class='darkmblue'>Аккаунт Discord уже привязан! Чтобы отвязать используйте команду <span class='boldannounce'>/бэй отвязать</span> в канале <b>#фан-зона<b> в Discord-сообществе!</span>")
		return
	...
	if(dbcon.IsConnected())
		...
		to_chat(usr, "<span class='darkmblue'>Для завершения используйте команду <span class='boldannounce'>/бэй привязать t:[token]</span> в канале <b>#фан-зона<b> в Discord-сообществе!</span>")
		load_player_discord(usr)
